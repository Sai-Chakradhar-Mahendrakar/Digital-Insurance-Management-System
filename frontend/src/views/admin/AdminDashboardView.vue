          ...
          <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              <div class="bg-blue-50 rounded-lg p-4">
                <div class="text-sm font-medium text-blue-600">Admin Role</div>
                <div class="text-lg font-bold text-blue-900">
                  {{ tokenInfo.payload?.role || 'N/A' }}
                </div>
              </div>
              <div class="bg-green-50 rounded-lg p-4">
                <div class="text-sm font-medium text-green-600">Email</div>
                <div class="text-sm font-bold text-green-900">
                  {{ tokenInfo.payload?.sub || 'N/A' }}
                </div>
              </div>
              <div class="bg-yellow-50 rounded-lg p-4">
                <div class="text-sm font-medium text-yellow-600">Session Expires</div>
                <div class="text-sm font-bold text-yellow-900">
                  {{ formatExpiry(tokenInfo.payload?.exp) }}
                </div>
              </div>
              <div class="bg-purple-50 rounded-lg p-4">
                <div class="text-sm font-medium text-purple-600">Time Remaining</div>
            ...
            <div class="mt-4">
              <button
                ...
                class="flex items-center text-sm text-slate-600 hover:text-slate-900 transition-colors"
              >
                <component
                  :is="showTokenDetails ? ChevronDown : ChevronRight"
                  class="w-4 h-4 mr-1"
                />
                {{ showTokenDetails ? 'Hide' : 'Show' }} Token Details
              </button>

              <div v-if="showTokenDetails" class="mt-4 bg-slate-100 rounded-lg p-4">
                <div class="text-sm font-mono text-slate-700">
                  <div class="mb-2"><strong>Payload:</strong></div>
                  ...
                  <div class="mt-4 grid grid-cols-2 gap-4 text-xs">
                    <div>
                      ...
                      {{ formatTimestamp(tokenInfo.payload?.iat) }}
                    </div>
                    <div>
                      <strong>Expires At:</strong><br />
                      {{ formatTimestamp(tokenInfo.payload?.exp) }}
                    </div>
                  </div>
                </div>
              </div>
